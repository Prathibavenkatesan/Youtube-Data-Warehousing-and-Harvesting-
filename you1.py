...
def create_table():
    cur.execute("create table if not exists channelinfo5(channel_id varchar(100) primary key, channel_name varchar(100),subscription_count int, channel_views int,channel_description text,playlist_id varchar(200))")
    cur.execute("create table if not exists playlistinfo5(playlistid varchar(100) primary key,playlistname varchar(100),channelid varchar(100),FOREIGN KEY (channelid) REFERENCES channelinfo5(channel_id))")
    cur.execute("create table if not exists videolistinfo5(video_id varchar(100)primary key, Video_name varchar(200),video_description text,Published_at varchar(100),View_count int,Like_count int,Favorite_count int,comment_count int,Duration varchar(100),caption_status varchar(300),channel_id varchar(100))")
    cur.execute("create table if not exists comments5(commentid varchar(100)primary key,videoid varchar(100),comment_text text,comment_author varchar(200),comment_published_date varchar(100),FOREIGN KEY (videoid) REFERENCES videolistinfo5(video_id))")
...
def channel_insert():
    ...
    for item, row in df.iterrows():
            insert_query = '''INSERT IGNORE INTO channelinfo5(channel_id, channel_name, subscription_count, channel_views, channel_description, playlist_id)
                              VALUES(%s, %s, %s, %s, %s, %s)'''
...
def playlist_insert():
    ...
    for item,row in df1.iterrows():
            insert_query='''INSERT IGNORE INTO playlistinfo5(playlistid,playlistname,channelid)
                                values(%s,%s,%s)'''
...
def video_insert():
    ...
    for item,row in df2.iterrows():
                insert_query1='''INSERT IGNORE INTO  videolistinfo5(video_id,Video_name,video_description,Published_at,View_count,Like_count,Favorite_count,comment_count,Duration,caption_status,channel_id)
...
def comment_insert():
    ...
    for item, row in df3.iterrows():
            insert_query4='''INSERT IGNORE INTO comments5(commentid,videoid,comment_text,comment_author,comment_published_date)
                            values(%s,%s,%s,%s,%s)'''
...
def show_channels():
    ...
    query="select * from channelinfo5"
    cur.execute(query)
    data=cur.fetchall()
...
def show_playlists():
    ...
    cur=mydb.cursor(dictionary=True)
    query="select * from playlistinfo5"
    cur.execute(query)
    data=cur.fetchall()
...
def show_videos():
    ...
    cur=mydb.cursor(dictionary=True)
    query="select * from videolistinfo5"
    cur.execute(query)
    data=cur.fetchall()
...
if selected=="Home":
    st.title("About")
    st.text('''This project builds a userfriendly streamlit application that utilizes the Google API to extract information on a YouTube channel, stores it in a SQL database, and enables users to search for channel details and join tables to view data in the Streamlit app.''')
        ...
        if input_channelid:
            ...
            if input_channelid not in ch_id:
                ...
                if insert:
                    ...
                    st.write("Data already exists in MongoDB")
            else:
                st.write("Data already exists in MongoDB")
    submit1=st.button("Migrate to sql")
    if submit1:
        cur.execute("select channel_name from channelinfo5")
        nch_id=cur.fetchall()
        if input_channelid not in nch_id:
            migration_status = table()
            if migration_status:
                st.success("Data saved into SQL")
            else:
                st.write("Data already stored into SQL")
        else:
            st.write("Data already stored into SQL")
...
if selected=="Insights":
    ...
    if ques=="1.What are the names of all the videos and their corresponding channels?":
        cur.execute("select channelinfo5.channel_name,videolistinfo5.video_name from channelinfo5 join videolistinfo5 on videolistinfo5.channel_id=channelinfo5.channel_id")
        ...
        cur.execute("select distinct c.channel_name,count(v.Video_name) as No_of_videos from channelinfo5 as c join videolistinfo5 as v on v.channel_id=c.channel_id group by c.channel_name order by No_of_videos desc")
        ...
        cur.execute("select c.channel_name,v.video_id,v.Video_name, v.view_count as View_count from channelinfo5 as c join videolistinfo5 as v on c.channel_id=v.channel_id group by v.video_id order by View_count desc limit 10")
        ...
        cur.execute("select Video_name,comment_count  from videolistinfo5")
        fetchdata3=cur.fetchall()
        df3=pd.DataFrame(fetchdata3,columns=["VIDEO_NAME","NO_OF_COMMENTS"])
        st.write(df3)
        
    elif ques=="5.What are the top 10 highest number of likes counts and what are their corresponding channel names?":
        cur.execute("select c.channel_name,v.Video_name,v.Like_count as Top_likecounts from channelinfo5 as c join videolistinfo5 as v on c.channel_id=v.channel_id order by Top_likecounts desc limit 10")
        ...
        cur.execute("select Video_name,Like_count from videolistinfo5")
        fetchdata5=cur.fetchall()
        df5=pd.DataFrame(fetchdata5,columns=["VIDEO_NAME","LIKE_COUNT"])
        st.write(df5)

    elif ques=="7.What is the total number of views for each channel, and what are their corresponding channel names?":
        cur.execute("select  channel_name,channel_views from channelinfo5")
        ...
        cur.execute("select c.channel_name,v.Video_name,v.Published_at from channelinfo5 as c join videolistinfo5 as v on c.channel_id=v.channel_id where Published_at=2022")
        fetchdata7=cur.fetchall()
        df7=pd.DataFrame(fetchdata7,columns=["CHANNEL_NAME","VIDEO_NAME","PUBLISHED_AT"])
        st.write(df7)
        ...
        cur.execute("""select c.channel_name,AVG(IFNULL(
            ...
            ) AS Average_Duration_Seconds from channelinfo5 as c join videolistinfo5 as v on c.channel_id=v.channel_id group by c.channel_name""")
        ...
        cur.execute("select c.channel_name,v.video_name,v.comment_count from channelinfo5 as c join videolistinfo5 as v on c.channel_id=v.channel_id order by comment_count desc")
